...
const requests = import("node-fetch")
...
//============================================================
    ...
    if (msg){
        ...
        if (cmd === "hi"){
            if (!modecmd(sender)) return
              //  wa.sendReply(to, "hai juga")
                wa.sendReplyWA(to, "Hey @!", "Dragon-J", [sender])
        ...
        */    mat += '⤷ Listhero fflogo\n'
            ...
            if (!modecmd(sender)) return
            wa.sendMessage(to, "hello buddy, What's Up?")
            printLogs(msg)
        } else if (cmd == "!leave") {
            ...
            let mids = []
            let fox = "*Tagging all Members of Group*\n"
            let no = 0
            for (let mem of xyz) {
                no += 1
                fox += "\n" + no + ". @!"
            ...
            fox += "\n\nName: "+ginfo.subject+"\nMember: "+mids.length+"\n\nDragon-J"
            wa.sendReplyWA(to, fox, "Dragon-J", mids)
            printLogs(msg)
            ...
            wa.sendReplyWA(to, fox, "Dragon-J", user)
            printLogs(msg)
